...
func NewServerConfig(config *config.Config) *Server {
	return &Server{
		GOCDHost:     config.GOCDHost,
		GOCDPort:     config.GOCDPort,
		GOCDUser:     config.GOCDUser,
		GOCDPassword: config.GOCDPassword,
	}
}

// GoCDURL returns the host of the GoCD server
func (server Server) GoCDURL() string {
	return fmt.Sprintf("%s:%s", server.GOCDHost, server.GOCDPort)
}
...
func (server Server) pipelineConfigPUT(pipeline Pipeline, etag string) (pipelineResult Pipeline, err error) {
	...
	req, err := http.NewRequest("PUT", server.GoCDURL()+"/go/api/admin/pipelines/"+pipelineName, payloadBody)
	if err != nil {
		return
	}

	req.SetBasicAuth(server.GOCDUser, server.GOCDPassword)
...
func (server Server) pipelineConfigPOST(pipelineConfig PipelineConfig) (pipeline Pipeline, err error) {
	...
	req, err := http.NewRequest("POST", server.GoCDURL()+"/go/api/admin/pipelines", payloadBody)
	if err != nil {
		return
	}
...
func (server Server) pipelineGET(pipelineName string) (pipeline Pipeline, etag string, err error) {
	req, err := http.NewRequest("GET", server.GoCDURL()+"/go/api/admin/pipelines/"+pipelineName, nil)
	if err != nil {
		return
	}
